...
// ---------------------------------------------------------------------------
// Post-generation fabrication check — catches technology/skill claims that
// cannot be traced back to the resume. Runs pure string processing, no API
// calls, so it adds negligible latency.
// ---------------------------------------------------------------------------

/**
 * Hardcoded list of specific technologies, tools, frameworks, languages,
 * databases, platforms, and certifications to check. Only concrete,
 * unambiguous tech terms — NOT soft skills, methodologies, or generic
 * domain concepts (those are too prone to false positives).
 *
 * Each entry is lowercased for comparison. Multi-word entries are matched
 * as whole phrases. Single-word entries are matched on word boundaries.
 */
const TECH_TERMS: readonly string[] = [
  // Programming languages
  "python", "javascript", "typescript", "java", "kotlin", "swift",
  "objective-c", "c++", "c#", "ruby", "go", "golang", "rust", "scala",
  "haskell", "erlang", "elixir", "clojure", "perl", "php", "r",
  "matlab", "julia", "dart", "lua", "groovy", "fortran", "cobol",
  "assembly", "solidity", "zig", "ocaml", "f#",
  // Frontend frameworks/libraries
  "react", "react.js", "reactjs", "next.js", "nextjs", "angular",
  "vue", "vue.js", "vuejs", "svelte", "sveltekit", "ember", "backbone",
  "jquery", "tailwind", "tailwindcss", "bootstrap", "material ui",
  "chakra ui", "styled-components", "redux", "mobx", "zustand",
  "htmx", "alpine.js",
  // Backend frameworks
  "node.js", "nodejs", "express", "express.js", "fastify", "nest.js",
  "nestjs", "django", "flask", "fastapi", "rails", "ruby on rails",
  ...
  "cassandra", "dynamodb", "couchdb", "neo4j", "sqlite", "mariadb",
  "cockroachdb", "tidb", "clickhouse", "snowflake", "bigquery",
  "supabase", "firestore", "firebase", "prisma", "drizzle",
  // Cloud / infra
  "aws", "amazon web services", "gcp", "google cloud", "azure",
  "heroku", "vercel", "netlify", "cloudflare", "digitalocean",
  "linode", "terraform", "pulumi", "ansible", "chef", "puppet",
  "vagrant",
  // DevOps / CI-CD
  "docker", "kubernetes", "k8s", "jenkins", "github actions",
  "gitlab ci", "circleci", "travis ci", "argo", "argocd", "helm",
  "istio", "envoy", "nginx", "apache",
  // Data / ML
  "tensorflow", "pytorch", "keras", "scikit-learn", "pandas", "numpy",
  "spark", "pyspark", "hadoop", "airflow", "dbt", "kafka",
  "rabbitmq", "flink", "beam", "mlflow", "kubeflow", "sagemaker",
  "hugging face", "langchain", "openai api",
  // Monitoring / observability
  "datadog", "grafana", "prometheus", "new relic", "splunk", "kibana",
  "sentry", "pagerduty",
  ...
  "junit", "rspec", "storybook",
  // API / protocols
  "graphql", "grpc", "rest api", "restful", "websocket", "protobuf",
  "trpc", "openapi", "swagger",
  // Certifications (commonly fabricated)
  "aws certified", "aws solutions architect", "google cloud certified",
  "azure certified", "pmp", "cfa", "cissp", "comptia", "cka",
  "ckad", "hashicorp certified",
  // Other tools / platforms
  "shopify", "stripe", "twilio", "sendgrid", "auth0", "okta",
  "salesforce", "hubspot", "segment", "amplitude", "mixpanel",
  "looker", "tableau", "power bi", "excel", "vba",
] as const;

/**
 * Builds a regex for a tech term that avoids false-positive substring
 * matches. Multi-word terms use plain case-insensitive includes. Single-
 * word terms use word-boundary matching.
 *
 * Special cases:
 * - "go" needs careful boundaries to avoid matching "going", "good", etc.
 * - "r" needs word boundaries to avoid matching inside any word.
 * - "c++" and "c#" need escaped special characters.
 * - ".net" and similar dotted names need escaped dots.
 */
function techTermRegex(term: string): RegExp {
  // Special ultra-short terms that need strict boundaries
  if (term === "go") {
    // Match "Go" as a standalone word but not inside "going", "good", "goal", etc.
    // Require it to be at a word boundary and not followed by common suffixes
    return /\bGo\b(?!ing|od|al|es|ne|t |ld|od|ver|ogl)/;
  }
  if (term === "r") {
    // "R" programming language — only match uppercase standalone R
    return /\bR\b(?!\.|esearch|eact|edis|uby|ails|est|ust)/;
  }

  // Escape regex special chars in the term
  const escaped = term.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");

  // For multi-word terms, just do a case-insensitive search (substring is fine
  // because multi-word phrases are specific enough)
  if (term.includes(" ") || term.includes(".") || term.includes("-")) {
    return new RegExp(escaped, "i");
  }

  // Single-word terms: word-boundary match
  return new RegExp(`\\b${escaped}\\b`, "i");
}

/**
 * Verifies that technology/skill/tool claims in the cover letter can be
 * traced back to the candidate's resume. Removes or rewrites sentences
 * containing fabricated claims.
 *
 * Returns the cleaned cover letter and a list of removed claims for logging.
 */
function verifyAgainstResume(
  coverLetter: string,
  resume: string
): { verified: string; removedClaims: string[] } {
  const removedClaims: string[] = [];

  // Pre-compute which tech terms appear in the resume (fast lookup set)
  const resumeTechSet = new Set<string>();
  for (const term of TECH_TERMS) {
    const regex = techTermRegex(term);
    if (regex.test(resume)) {
      resumeTechSet.add(term);
    }
  }

  // Split cover letter into paragraphs, then sentences, so we can surgically
  // remove individual sentences without destroying paragraph structure.
  const paragraphs = coverLetter.split(/\n\n+/);
  const cleanedParagraphs: string[] = [];

  for (const paragraph of paragraphs) {
    // Split on sentence boundaries: a period followed by whitespace. The
    // lookbehind keeps the period attached to the preceding sentence.
    const sentenceParts = paragraph.split(/(?<=\.)\s+/);
    const keptSentences: string[] = [];

    for (const sentence of sentenceParts) {
      const sentenceLower = sentence.toLowerCase();
      let hasFabricatedClaim = false;
      let fabricatedTerm = "";

      for (const term of TECH_TERMS) {
        const regex = techTermRegex(term);
        if (!regex.test(sentence)) continue;  // Term not in this sentence
        if (resumeTechSet.has(term)) continue; // Term IS in the resume — legit

        // Term is in the sentence but NOT in the resume. Check if this sentence
        // is actually CLAIMING the candidate has this skill, vs. merely
        // referencing the company's stack or expressing interest in learning.
        //
        // Heuristic: if the sentence contains possessive/proficiency markers
        // alongside the tech term, it's a fabricated claim. If it's framed as
        // the company's need or a learning goal, it's fine.
        const claimPatterns = [
          // Direct action claims: "I built/developed/shipped/etc. [with] X"
          /\b(?:i[''\u2019]?ve|i have|i[''\u2019]?m|i am|i developed|i built|i designed|i created|i implemented|i wrote|i shipped|i deployed|i managed|i led|i optimized|i maintained|i used|i worked with|i worked in)\b/i,
          // Skill/experience possession claims: "my X skills", "proficient in X"
          /\b(?:my experience (?:with|in)|my skills? in|my expertise in|my proficiency in|my background in|my knowledge of|proficient in|skilled in|experienced (?:with|in)|familiar with|fluent in|hands-on (?:with|experience)|developed (?:a background|expertise|proficiency|skills?) in)\b/i,
          // Possessive framing that implies ownership: "my Python work", "a tech stack I'm familiar with"
          /\b(?:my .{0,30}(?:skills?|expertise|proficiency|experience|background|knowledge|work)|a tech stack i)\b/i,
        ];

        const isClaimingSKill = claimPatterns.some((p) => p.test(sentenceLower));

        // Also check for "interest/learning" framing which is NOT a fabrication
        const interestPatterns = [
          /\b(?:learn|learning|interested in|want to|looking to|eager to learn|grow into|ramp up on|pick up|transition to)\b/i,
          // Company-referencing patterns: "your team uses X", "[Company] requires X"
          /\b(?:your|you['']?re|the (?:role|position|team|company) (?:requires|needs|uses|involves))\b/i,
        ];

        const isInterestOrCompanyRef = interestPatterns.some((p) =>
          p.test(sentenceLower)
        );

        if (isClaimingSKill && !isInterestOrCompanyRef) {
          hasFabricatedClaim = true;
          fabricatedTerm = term;
          break; // One fabricated claim is enough to flag the sentence
        }
      }

      if (hasFabricatedClaim) {
        removedClaims.push(`"${sentence.trim()}" [fabricated: ${fabricatedTerm}]`);
        // Don't add this sentence to keptSentences — it's removed
      } else {
  ...
  let verified = cleanedParagraphs.join("\n\n");

  // Clean up artifacts from sentence removal
  verified = verified
    // Double spaces
    .replace(/ {2,}/g, " ")
    // Orphaned leading conjunctions after sentence removal
    .replace(/(?:^|\.\s+)(?:And|But|Also|Additionally|Furthermore|Moreover)\s+/gm, "")
    // Capitalize after period if removal left lowercase
    .replace(/\.\s+([a-z])/g, (_m, letter: string) => `. ${letter.toUpperCase()}`)
    // Leading/trailing whitespace per paragraph
    .split(/\n\n+/)
    .map((p) => p.trim())
    .filter((p) => p.length > 0)
    .join("\n\n")
    ...
    const readable = new ReadableStream({
      async start(controller) {
        try {
          ...
          // Post-process: verify tech/skill claims against resume
          const { verified, removedClaims } = verifyAgainstResume(
            cleaned,
            resume
          );
          ...
          if (removedClaims.length > 0) {
            console.warn(
              `[ApplyFaster] Fabrication check: removed ${removedClaims.length} unverified claim(s):\n` +
                removedClaims.map((c) => `  - ${c}`).join("\n")
            );
          }
